...
## Get the Data
####
bike <- read.csv('bikeshare.csv')  # Before running this line, set working directory in the RStudio Files panel -> More
head(bike)
str(bike)
summary(bike)
...
# Plot count vs. temp
ggplot(bike, aes(x=temp,y=count)) + geom_point(alpha=0.2, color='light blue') + theme_bw()
...
ggplot(filter(bike,workingday==1), aes(hour,count)) +
       ...
       theme_bw()
# position=position_jitter(w=1, h=0) adds jitter. Width = 1 and height = 0.
...
# We notice that this sort of model doesn't work well given our 
